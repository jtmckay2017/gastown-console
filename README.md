...
| **Plans** | The Board says a plan exists; this is where one gets read. Every bead carrying a proposed plan or acceptance criteria, newest first, and the plan itself at full width with its structure rendered — headings, numbered states, sub-points, monospace runs — instead of dumped as a block. Send an agent feedback on it or revise it yourself without leaving it, and if an agent rewrites it while you are reading, the page says so and says which parts moved |
