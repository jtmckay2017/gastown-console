...
| **Board** | The same beads as a kanban — columns are beads' own statuses (blocked always among them), plus one derived from the agents' screens: "Being worked", because nothing in Gas Town ever writes `in_progress` and a column keyed on it stands empty while polecats work. Swimlanes are the epic hierarchy, and clicking a card opens its gathered context, proposed plan and acceptance criteria beside the board. Where the console writes: draft a bead, revise the plan on one, link two, mail an agent to go and redraft it — and, on a console bound to your own machine, read a finished plan and approve it, which puts an agent on it. The same block of actions is on the Plans tab, which is the other place a plan gets read |
| **Plans** | The Board says a plan exists; this is where one gets read. Every bead carrying a proposed plan or acceptance criteria, newest first, and the plan itself at full width with its structure rendered — headings, numbered states, sub-points, monospace runs — instead of dumped as a block. It opens on the plans that are **not closed**, because a town finishes most of what it plans and the finished ones bury the live ones; filter by status (`bd`'s own words) or by rig, search by title, id or assignee, and the count says how many plans the filters are holding back. Send an agent feedback on one, revise it yourself, or — on a console bound to your own machine — approve it and put an agent on it, all without leaving the plan you are reading; and if an agent rewrites it while you are reading, the page says so and says which parts moved |
...
would quietly drop their work. The Board and the Plans tab both open these forms; they are
the same endpoints and the same one implementation of that check, reached from two places.
